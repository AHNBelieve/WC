...
function WeatherChart() {
    ...
    useEffect(() => {
        const fetchData = async () => {
            try {
                const response = await fetch('https://api.openweathermap.org/data/2.5/forecast?lat=37.12988&lon=127.0672452&appid=674bc744607f1dcc62ba480a79132421&units=metric'); // API URL 입력
                const weatherData = await response.json();

                const formattedData = weatherData.list.map((item: ListItem) => ({
                    date: formatDate(item.dt_txt),
                    temperature: item.main.temp,
                    fullDate: new Date(item.dt * 1000).toLocaleDateString('ko-KR', { month: 'numeric', day: 'numeric' }), // 전체 날짜 추가
    ...
    const formatTime = (dateString: string) => {
        ...
        return date.toLocaleTimeString('en-US', { hour: '2-digit', minute: '2-digit', hour12: true }); // 시간 포맷
    };
    ...
    const CustomTick = ({ x, y, payload }: { x: number; y: number; payload: any }) => (
        <text x={x} y={y} textAnchor="middle" fill="#000000" fontSize={15} fontWeight="bold">
            {payload.value}
        </text>
    );
    // 툴팁 커스텀
    ...
    return (
        <div className={style.container} >
            <ResponsiveContainer width="100%" height={204} style={{ padding: '10px' }}>
                <LineChart data={displayedData} margin={{ left: -40 }}>
                    ...
                    <Line
                        ...
                        dataKey="temperature"
                        stroke="#000000"
                        label={({ x, y, value }) => ( // x, y, value를 직접 받음
                            <text x={x} y={y - 15} textAnchor="middle" fill="#000">{`${value}°C`}</text>
                        )}
                    />
                </LineChart>
            </ResponsiveContainer>
            ...
            <div className={style.button}>
                ...
                <FaChevronRight
                    onClick={handleNext}
                    style={{ opacity: currentPage >= Math.ceil(data.length / itemsPerPage) - 1 ? 0.5 : 1, cursor: currentPage >= Math.ceil(data.length / itemsPerPage) - 1 ? 'default' : 'pointer' }}
                />
            </div>
        </div >
    );
